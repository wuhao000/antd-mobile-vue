...
export const useDatePickerBase = (props, { emit }, { genMonthComponent }) => {
    ...
    const genMonthData = (date, addMonth = 0) => {
        let copyDate = date;
        if (!copyDate) {
    ...
    const inDate = (date, tick) => {
        return date <= tick && tick < date + 24 * 3600000;
    };
    const selectDateRange = (startDate, endDate, clear = false) => {
        ...
        state.months.filter(m => {
            ...
            m.weeks.forEach(w => w.filter(d => {
                ...
                if (clear) {
                    ...
                    const info = getDateExtra && getDateExtra(new Date(d.tick), [...currentValue.value]) || {};
    ...
    const computeVisible = (clientHeight, scrollTop) => {
        let needUpdate = false;
