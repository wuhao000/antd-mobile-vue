...
let ImagePicker = class ImagePicker extends Vue {
    ...
    render() {
        const { prefixCls, value = [], selectable, multiple, accept } = this;
        const imgItemList = [];
        ...
        value.forEach((image, index) => {
            ...
            const itemStyle = {};
            imgItemList.push(<Flex.Item key={`item-${index}`} style={itemStyle}>
            <div key={index} class={`${prefixCls}-item`}>
              <div class={`${prefixCls}-item-remove`} role="button" aria-label="Click and Remove this image" 
            ...
            onClick={() => {
              ...
              {
            // @ts-ignore
            <HtmlComponent class={`${prefixCls}-item-content`} role="button" aria-label="Image can be clicked" 
            // tslint:disable-next-line:jsx-no-multiline-js
            onClick={() => {
                this.onImageClick(image, index);
            }} html={imgStyle.backgroundImage === 'none' ? getIcon(image) : null} style={imgStyle}>
                </HtmlComponent>}
            </div>
          </Flex.Item>);
        });
        const selectEl = (<Flex.Item key="select">
          <TouchFeedback activeClassName={`${prefixCls}-upload-btn-active`}>
            <div class={`${prefixCls}-item ${prefixCls}-upload-btn`} onClick={this.onImageClick} role="button" aria-label="选择并添加图片">
              <input ref="fileSelectorInput" type="file" accept={accept} onchange={(v) => {
            this.onFileChange(v);
        }} multiple={multiple}/>
            </div>
          </TouchFeedback>
...
], ImagePicker.prototype, "prefixCls", void 0);
__decorate([
    Prop({
        default: () => {
            return [];
...
], ImagePicker.prototype, "value", void 0);
__decorate([
    Prop({
        type: Boolean,
        default: true
    })
], ImagePicker.prototype, "selectable", void 0);
__decorate([
    Prop({
        type: Boolean,
        default: false
    })
], ImagePicker.prototype, "multiple", void 0);
__decorate([
    Prop({
        type: String,
        default: '*'
    })
], ImagePicker.prototype, "accept", void 0);
__decorate([
    Prop({ type: Number, default: 4 })
], ImagePicker.prototype, "length", void 0);
__decorate([
    Prop({ type: Number, default: 8 })
], ImagePicker.prototype, "maxLength", void 0);
__decorate([
    Prop({ type: Number, default: 0 })
], ImagePicker.prototype, "maxSize", void 0);
