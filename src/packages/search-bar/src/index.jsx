...
export default defineComponent({
    ...
    setup(props, { emit, attrs, slots }) {
        const rightBtnInitMarginleft = ref(null);
        const firstFocus = ref(null);
        const blurFromOnClear = ref(null);
        const onBlurTimeout = ref(null);
        ...
        const update = () => {
            if (syntheticPhRef.value) {
                if (inputContainerRef.value &&
                    inputContainerRef.value.className.indexOf(`${props.prefixCls}-start`) > -1) {
    ...
    render() {
        ...
        return (<form onSubmit={this.onSubmit} class={wrapCls} ref={this.inputContainerRef} action="#">
        <div class={`${prefixCls}-input`}>
          <div class={`${prefixCls}-synthetic-ph`} ref={this.syntheticPhRef}>
            <span class={`${prefixCls}-synthetic-ph-container`} ref={this.syntheticPhContainerRef}>
        ...
        style={{
          ...
          <input type="search" class={`${prefixCls}-value`} value={value} disabled={disabled} placeholder={placeholder} onInput={this.onChange} onChange={this.onChange} onFocus={this.onFocus} onBlur={this.onBlur} ref={this.inputRef} maxlength={maxLength}/>
          <TouchFeedback2 activeclass={`${prefixCls}-clear-active`}>
            <a onClick={this.onClear} class={clearCls}/>
          </TouchFeedback2>
        </div>
        <div class={cancelCls} onClick={this.onCancel} ref={this.rightBtnRef}>
