...
const CarouselBase = defineComponent({
    ...
    setup(props, { emit, slots }) {
        const touchObject = ref({});
        const autoplayPaused = ref(null);
        const clickSafe = ref(true);
        ...
        const setState = (object, callback) => {
        ...
        const getTouchEvents = () => {
            ...
            return {
                ...
                onTouchmove(e) {
                    const direction = swipeDirection(touchObject.value.startX, e.touches[0].pageX, touchObject.value.startY, e.touches[0].pageY);
                    if (direction !== 0) {
                        e.preventDefault();
                    }
                    const length = props.vertical ? Math.round(Math.sqrt(Math.pow(e.touches[0].pageY - touchObject.value.startY, 2))) : Math.round(Math.sqrt(Math.pow(e.touches[0].pageX - touchObject.value.startX, 2)));
                    touchObject.value = {
        ...
        const getMouseEvents = () => {
            ...
            return {
                ...
                onMousemove(e) {
                    ...
                    const direction = swipeDirection(touchObject.value.startX, e.clientX, touchObject.value.startY, e.clientY);
                    if (direction !== 0) {
                        e.preventDefault();
                    }
                    const length = props.vertical ? Math.round(Math.sqrt(Math.pow(e.clientY - touchObject.value.startY, 2))) : Math.round(Math.sqrt(Math.pow(e.clientX - touchObject.value.startX, 2)));
                    touchObject.value = {
                        startX: touchObject.value.startX,
                        startY: touchObject.value.startY,
                        endX: e.clientX,
        ...
        const handleSwipe = (_) => {
            ...
            if (childrenCount() > 1 && touchObject.value.length > (state.slideWidth / slidesToShow) / swipeSpeed) {
                if (touchObject.value.direction === 1) {
                    if (state.currentSlide >= childrenCount() - slidesToShow &&
                        !props.wrapAround) {
        ...
        const animateSlide = (easing, duration, endValue, callback) => {
        ...
        const getTargetLeft = (touchOffset, slide) => {
            let offset;
        ...
        const formatChildren = (children) => {
            ...
            return realChildren.map((child, index) => {
                return <li class="slider-slide" style={getSlideStyles(index, positionValue)} key={index}>{child}</li>;
            });
        };
        ...
        const setDimensions = () => {
            ...
            const firstSlide = filterNotEmpty(frame && frame.childNodes[0].childNodes)[0];
            if (firstSlide) {
    ...
    render() {
        ...
        <div class="slider-frame" ref={this.setFrameRef} style={this.getFrameStyles()} {...Object.assign(Object.assign({}, this.getTouchEvents()), this.getMouseEvents())} onClick={this.handleClick.bind(this)}>
          <ul class="slider-list" ref="list" style={this.getListStyles()}>
            {children}
          </ul>
        </div>
        {this.decorators ?
            this.decorators.map((Decorator, index) => (<div style={Object.assign(Object.assign({}, this.getDecoratorStyles(Decorator.position)), (Decorator.style || {}))} class={'slider-decorator-' + index} key={index}>
              <Decorator.component {...{
