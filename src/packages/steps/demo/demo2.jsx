...
export default defineComponent({
    render() {
        return (<WingBlank mode={20} class="stepsExample">
        <div class="sub-title">Horizontal small size</div>
        <WhiteSpace />
        <Steps current={1} direction="horizontal" size="small">{steps.map(it => <Step {...it}/>)}</Steps>
        <div class="sub-title">Horizontal default size</div>
        <WhiteSpace />
        <Steps current={1} direction="horizontal">{steps.map(it => <Step {...it}/>)}</Steps>
