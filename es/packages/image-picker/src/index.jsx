    ...
    render() {
        ...
        value.forEach((image, index) => {
            ...
            onClick={() => {
              ...
              <div class={`${prefixCls}-item-content`} role="button" aria-label="Image can be clicked" 
            // tslint:disable-next-line:jsx-no-multiline-js
            onClick={() => {
        ...
        const selectEl = (<Flex.Item key="select">
          <TouchFeedback activeClassName={`${prefixCls}-upload-btn-active`}>
            <div class={`${prefixCls}-item ${prefixCls}-upload-btn`} onClick={this.onImageClick} role="button" aria-label="Choose and add image">
              <input ref={'fileSelectorInput'} type="file" accept={accept} 
        // tslint:disable-next-line:jsx-no-multiline-js
        onchange={this.onFileChange} multiple={multiple}/>
            </div>
          </TouchFeedback>
        </Flex.Item>);
...
ImagePicker = tslib_1.__decorate([
    Component({
        name: 'ImagePicker'
    })
], ImagePicker);
